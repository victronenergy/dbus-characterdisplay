...
class Page(Tracker):
	...
	def __new__(klass):
		klass._instance = super(Page, klass).__new__(klass)
		return klass._instance
...
class InverterInfoPage(Page):
	...
	def get_battery_text(self):
		if None in (self.cache.batterypower, self.cache.state):
			return " "*DISPLAY_COLS
		return "\004" + \
			("{:.0f}%".format(self.cache.soc) if self.cache.soc is not None else '---') + \
			" " + self.format_power(self.cache.batterypower, 5) + "W " + \
			self.states.get(self.charge_state, "---")
...
class BatteryPage(Page):
	...
	def get_text(self, conn):
		return "{:>3s}  {:>3s}%  {:>4s}V\n".format(
			...
			"{:>5s}W {:>3s}".format(self.format_power(self.cache.battery_power, 5) if self.cache.battery_power is not  None else "---", self.states.get(self.cache.battery_state, "---"))
